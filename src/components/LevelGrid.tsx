        ...
        <div className="p-4 h-full flex flex-col">
            ...
            >
                <div className="space-y-8">
                    {[
                        { name: 'Beginner', range: [1, 10] },
                        { name: 'Apprentice', range: [11, 20] },
                        { name: 'Warrior', range: [21, 30] },
                        { name: 'Artisan', range: [31, 40] },
                        { name: 'Voyager', range: [41, 50] },
                        { name: 'Master', range: [51, 60] },
                        { name: 'Zen', range: [61, 70] },
                        { name: 'Legend', range: [71, 80] },
                    ].map((category) => {
                        const categoryLevels = levels.filter(l => l.id >= category.range[0] && l.id <= category.range[1]);
                        if (categoryLevels.length === 0) return null;

                        return (
                            <div key={category.name} className="flex flex-col gap-4">
                                <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500 border-l-2 border-indigo-500/30 pl-3">
                                    {category.name}
                                </h2>
