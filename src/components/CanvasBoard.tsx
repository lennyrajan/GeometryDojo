...
export const CanvasBoard: React.FC<CanvasBoardProps> = ({ level, difficulty, onComplete, onNext, theme = 'space' }) => {
    ...
    const draw = () => {
        ...
        // Draw User Path (Raw)
        // Show if drawing OR if replaying OR (if result exists but we want to show underlying path? No, result view replaces it)
        // Logic:
        // 1. Drawing: Show current points.
        // 2. Replaying: Show points up to replayProgress.
        // 3. Result Static: Show Heatmap (handled below).
        if ((points.length > 0 && !result) || isReplaying) {
            ctx.beginPath();
            ...
            const limit = isReplaying ? replayProgress : points.length;
        ...
        // Only draw Heatmap if NOT replaying
        if (!isReplaying) {
            ...
            for (let i = 0; i < alignedCanvas.length - 1; i++) {
                ctx.beginPath();
                const diff = diffs[i];
                const goodness = Math.max(0, 1 - (diff * 20)); // Sensitivity
                const hue = goodness * 120; // 0 (Red) to 120 (Green)
...
// Replay Logic
const startReplay = () => {
    // We replay the RAW points, not the processed result
    if (points.length === 0) return;
    setIsReplaying(true);
    setReplayProgress(0);

    const totalPoints = points.length;
    const speed = Math.max(1, Math.floor(totalPoints / 60)); // target ~1 sec duration
